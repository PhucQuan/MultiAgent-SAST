"""
PHP language plugin for Aegis-SAST.

Implements security analysis for PHP code using Tree-sitter.
...
class PHPPlugin(ILanguagePlugin):
    """PHP language analysis plugin using Tree-sitter."""
    ...
    def extract_sources(self, ast: Any, file_path: Path, rules: Dict[str, Any]) -> List[TaintSource]:
        ...
        def visit_node(node: Node):
            ...
            for rule in source_rules:
                ...
                # For PHP we look for variable by name ($_ is superglobal)
                if pattern in node_text and node.type in ["variable_name", "subscript_expression", "function_call_expression"]:
    ...
    def extract_sinks(self, ast: Any, file_path: Path, rules: Dict[str, Any]) -> List[TaintSink]:
        ...
        sink_rules = []
        for category in ["sqli", "rce", "path_traversal", "xss", "ssrf", "xxe", "deserialization"]:
        ...
        def visit_node(node: Node):
            ...
            for rule in sink_rules:
                pattern = rule.get("pattern", "")
                if node.type == "function_call_expression" and pattern in node_text:
                    child_call_matches = any(
                        c.type == "function_call_expression" and pattern in c.text.decode('utf-8', errors='replace')
    ...
    def extract_sanitizers(self, ast: Any, file_path: Path, rules: Dict[str, Any]) -> List[Sanitizer]:
        ...
        def visit_node(node: Node):
            ...
            for rule in sanitizer_rules:
                pattern = rule.get("pattern", "")
                if pattern in node_text and node.type == "function_call_expression":
                    location = CodeLocation(
                        file_path=str(file_path),
                        line_number=node.start_point[0] + 1,
                        column_number=node.start_point[1],
    ...
    def track_dataflow(self, ast: Any, file_path: Path, source: TaintSource,
        ...
        if source_ast_node:
            ...
            while curr:
                if curr.type in ["function_definition", "method_declaration"]:
                    scope_node = curr
                    break
                curr = curr.parent
        ...
        def analyze_assignments(node: Node, depth: int = 0):
            ...
            if node.type == "assignment_expression":
                ...
                if left and right:
                    ...
                    words = re.findall(r'[\$\w]+', right_text)
                ...
                words = re.findall(r'[\$\w]+', call_text)
                if any(tvar in words for tvar in tainted_vars):
                    for sink in sinks:
                        if sink.location.line_number == node.start_point[0] + 1:
                            ...
                            paths.append(DataFlowPath(source=source, sink=sink, intermediate_steps=[], sanitizers=path_sanitizers))
            for child in node.children:
    ...
    def _extract_variable_name(self, node: Node) -> str:
        ...
        while current:
            if current.type == "assignment_expression":
                return current.children[0].text.decode('utf-8', errors='replace').strip()
            if current.type in ["function_definition", "program"]:
                break
            current = current.parent
        # Try to extract PHP variable like $_GET['id']
